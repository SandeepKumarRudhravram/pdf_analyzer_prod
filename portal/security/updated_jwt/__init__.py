...
CONFIG_DEFAULTS = {
    ...
    'JWT_AUTH_URL_RULE': '/PDFAnalyzerX/auth',
...
def iail_encryption(s, d):
    tmp = s[d:] + s[0: d]
    return tmp


def iail_decryption(s, d):
    return iail_encryption(s, len(s) - d)
...
def _default_auth_response_handler(access_token, identity):
    ...
    s = str(new_token[2])
    k = 2
    p = iail_encryption(s, k)

    new_encoded_token = str(new_token[0]) + "." + str(new_token[1]) + "." + str(p)
    return jsonify({'access_token': new_encoded_token})
...
def _jwt_required(realm):
    ...
    """
    # print("request call",_jwt.request_callback())
    if _jwt.request_callback() is None:
        raise JWTError('Authorization Required', 'Request does not contain an access token',
                       headers={'WWW-Authenticate': 'JWT realm="%s"' % realm})
    token_for_decr = str(_jwt.request_callback()).split(".")
    k = 2

    z = iail_decryption(token_for_decr[2], k)
    new_token_for_decr = str(token_for_decr[0]) + "." + str(token_for_decr[1]) + "." + str(z)
    token = new_token_for_decr
    # print("my token after decr",token)
